...
TEST(E2E_Phase3, ComponentLeadCoordinatesMultipleModules) {
  ...
  module_lead_1->setAvailableTaskAgents({task_agents[0]->getAgentId(),
                                         task_agents[1]->getAgentId(),
                                         task_agents[2]->getAgentId()});

  // module_lead_2 gets task_4, task_5, task_6
  module_lead_2->setAvailableTaskAgents({task_agents[3]->getAgentId(),
                                         task_agents[4]->getAgentId(),
                                         task_agents[5]->getAgentId()});

  std::cout << "\n=== Phase 3 E2E Test: Full 4-Layer Hierarchy ==="
            << std::endl;
  ...
  auto msg_to_component = KeystoneMessage::create(
      chief->getAgentId(), component_lead->getAgentId(), component_goal);

  std::cout << "1. ChiefArchitect → ComponentLead: " << component_goal
            << std::endl;
  ...
  auto component_response =
      component_lead->processMessage(*component_msg).get();
  ...
  std::cout << "   - ModuleLead 1 (Messaging) receives: "
            << module_1_msg->command << std::endl;
  ...
  std::cout << "   - ModuleLead 2 (Concurrency) receives: "
            << module_2_msg->command << std::endl;
  ...
  for (auto& agent : task_agents) {
    ...
    if (task_msg.has_value()) {
      std::cout << "   - " << agent->getAgentId()
                << " processes: " << task_msg->command << std::endl;
  ...
  auto msg_from_mod1 = KeystoneMessage::create(
      module_lead_1->getAgentId(), component_lead->getAgentId(),
      "module_result", module_1_result);
  module_lead_1->sendMessage(msg_from_mod1);

  auto msg_from_mod2 = KeystoneMessage::create(
      module_lead_2->getAgentId(), component_lead->getAgentId(),
      "module_result", module_2_result);
  ...
  auto final_msg =
      KeystoneMessage::create(component_lead->getAgentId(), chief->getAgentId(),
  ...
  for (const auto& agent : task_agents) {
    ...
    EXPECT_EQ(history.size(), 1u)
        << agent->getAgentId() << " should execute 1 task";
  }
