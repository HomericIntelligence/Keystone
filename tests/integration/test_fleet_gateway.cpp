...
class Child {
 public:
  explicit Child(const std::vector<std::string>& arguments) {
    ...
    const int spawned = posix_spawnp(&pid_, argv[0], &actions, nullptr, argv.data(), environ);
...
class FleetGateway : public ::testing::Test {
 protected:
  void SetUp() override {
    ...
    ASSERT_EQ(getsockname(socket_fd, reinterpret_cast<sockaddr*>(&address), &size), 0);
    const auto port = std::to_string(ntohs(address.sin_port));
    close(socket_fd);
    url_ = "nats://127.0.0.1:" + port;
    server_ = std::make_unique<Child>(std::vector<std::string>{
        "nats-server", "-js", "-a", "127.0.0.1", "-p", port, "-sd", directory_});
    ...
    ASSERT_EQ(js_AddConsumer(nullptr, js_, "homeric-myrmidon", &consumer, nullptr, nullptr),
              NATS_OK);
  }
  ...
  std::unique_ptr<Child> gateway(const std::string& consumer = "agent-one") {
    return std::make_unique<Child>(std::vector<std::string>{FLEET_GATEWAY_BINARY,
                                                            "--nats-url",
                                                            url_,
                                                            "--allow-loopback-test",
                                                            "--stream",
                                                            ...
                                                            consumer,
                                                            "--subject",
                                                            "hi.myrmidon.cpp.contributor.task.*",
                                                            "--publish-prefix",
                                                            "hi.fleet.events.worker-one",
                                                            "--worker-id",
                                                            "worker-one",
                                                            "--generation",
                                                            "7"});
...
TEST_F(FleetGateway, DeliversOpaqueEnvelopeAndAcknowledgesOnlyOnRequest) {
  ...
  EXPECT_EQ(response.at("subject"), "hi.myrmidon.cpp.contributor.task.task-one");
  EXPECT_EQ(response.at("numDelivered"), 1);
  const auto delivery = response.at("deliveryId");
  EXPECT_EQ(pull(*child).at("ok"), false);
  EXPECT_EQ(child->request({{"operation", "ack"}, {"deliveryId", "foreign"}}).at("ok"), false);
  EXPECT_EQ(child->request({{"operation", "inProgress"}, {"deliveryId", delivery}}).at("ok"), true);
  EXPECT_EQ(child->request({{"operation", "ack"}, {"deliveryId", delivery}}).at("ok"), true);
  jsConsumerInfo* info{};
  ASSERT_EQ(js_GetConsumerInfo(&info, js_, "homeric-myrmidon", "agent-one", nullptr, nullptr),
            NATS_OK);
  ...
  EXPECT_EQ(Json(child->observations).dump().find("DO-NOT-LOG-SECRET"), std::string::npos);
...
TEST_F(FleetGateway, DetachDoesNotAckAndReconnectUsesTheSameDurable) {
  ...
  EXPECT_EQ(
      second->request({{"operation", "ack"}, {"deliveryId", delivered.at("deliveryId")}}).at("ok"),
      false);
  EXPECT_EQ(second->request({{"operation", "ack"}, {"deliveryId", redelivered.at("deliveryId")}})
                .at("ok"),
            true);
}
...
TEST_F(FleetGateway, RejectsMissingConsumerWithoutCreatingOne) {
  ...
  jsConsumerInfo* info{};
  EXPECT_EQ(js_GetConsumerInfo(&info, js_, "homeric-myrmidon", "missing-agent", nullptr, nullptr),
            NATS_NOT_FOUND);
  jsConsumerInfo_Destroy(info);
}
...
TEST_F(FleetGateway, NakPermitsBrokerRedelivery) {
  ...
  EXPECT_EQ(child->request({{"operation", "nak"}, {"deliveryId", first.at("deliveryId")}}).at("ok"),
            true);
...
TEST_F(FleetGateway, InProgressDoesNotCompleteTheMessage) {
  ...
  for (int count = 0; count < 3; ++count) {
    ...
    EXPECT_EQ(child->request({{"operation", "inProgress"}, {"deliveryId", first.at("deliveryId")}})
                  .at("ok"),
              true);
  }
  jsConsumerInfo* info{};
  ...
  jsConsumerInfo_Destroy(info);
  EXPECT_EQ(child->request({{"operation", "ack"}, {"deliveryId", first.at("deliveryId")}}).at("ok"),
            true);
}
...
TEST_F(FleetGateway, SeparateLogicalConsumersCanHoldWorkConcurrently) {
  ...
  consumer.AckPolicy = js_AckExplicit;
  consumer.MaxAckPending = 1;
  ASSERT_EQ(js_AddConsumer(nullptr, js_, "homeric-myrmidon", &consumer, nullptr, nullptr), NATS_OK);
  publishWork();
  auto first = gateway();
...
TEST_F(FleetGateway, IncompleteFrameCannotPublishOnEof) {
  ...
  jsStreamInfo* info{};
  ASSERT_EQ(js_GetStreamInfo(&info, js_, "homeric-myrmidon", nullptr, nullptr), NATS_OK);
