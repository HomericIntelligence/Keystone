...
void AgentCore::receiveMessage(const core::KeystoneMessage& msg) {
  ...
  bool should_apply_backpressure =
      (total_depth >= core::Config::AGENT_MAX_QUEUE_SIZE);
  bool was_backpressure_applied =
      backpressure_applied_.load(std::memory_order_relaxed);
  ...
  if (should_apply_backpressure) {
    ...
    if (!was_backpressure_applied) {
      ...
      bool expected = false;
      if (backpressure_applied_.compare_exchange_strong(
...
std::optional<core::KeystoneMessage> AgentCore::getMessage() {
  ...
  int64_t last_check_ns =
      last_low_priority_check_ns_.load(std::memory_order_acquire);
  int64_t interval_ns =
