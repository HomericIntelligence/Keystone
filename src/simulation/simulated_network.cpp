...
void SimulatedNetwork::send(size_t from_node, size_t to_node,
  ...
  if (shouldDropPacket()) {
    dropped_messages_++;
    Logger::debug("SimulatedNetwork: Packet dropped ({}→{})", from_node,
                  to_node);
    return;
  }
...
std::optional<std::function<void()>> SimulatedNetwork::receive(size_t node_id) {
  ...
  // Deliver message
  auto latency_us =
      std::chrono::duration_cast<std::chrono::microseconds>(now - msg.sent_at)
          .count();
...
void SimulatedNetwork::healPartition() {
  ...
  Logger::info(
      "SimulatedNetwork: Partition healed - full connectivity restored");
}

bool SimulatedNetwork::isPartitioned() const { return is_partitioned_.load(); }
...
bool SimulatedNetwork::canCommunicate(size_t from_node, size_t to_node) const {
  ...
  bool from_in_a = std::find(partition_a_.begin(), partition_a_.end(),
                             from_node) != partition_a_.end();
  bool to_in_a = std::find(partition_a_.begin(), partition_a_.end(), to_node) !=
                 partition_a_.end();
  ...
  bool from_in_b = std::find(partition_b_.begin(), partition_b_.end(),
                             from_node) != partition_b_.end();
  bool to_in_b = std::find(partition_b_.begin(), partition_b_.end(), to_node) !=
                 partition_b_.end();
